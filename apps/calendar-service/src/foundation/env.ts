...
const ServiceEnvSchema = BaseNodeEnvSchema.extend({
  SERVICE_NAME: z.literal("calendar-service").default("calendar-service"),
  PORT: z.coerce.number().int().min(1).max(65535).default(4013),
...
export function loadServiceEnv(): ServiceEnv {
  ...
  return createEnv(ServiceEnvSchema, {
    source: {
      ...process.env,
      PORT: process.env.PORT ?? process.env.CALENDAR_SERVICE_PORT
    }
  });
}
