...
import { registerCalendarRoutes } from "../src/modules/events/events.routes.js";

test("calendar sync returns 501 when Calendar is connected but live Corsair sync is not implemented", async () => {
  const app = Fastify({ logger: false });
  app.setErrorHandler((error, _request, reply) => {
    reply.code(statusCode(error)).send({ error: { code: errorCode(error), message: error.message } });
  });
  await registerCalendarRoutes(app, { db: connectedFakeDb() });
  ...
  assert.match(response.body, /Live Calendar sync is not configured/);
  await app.close();
});
